...
typedef struct SFlashState
{
	EFlashState eState;
	uint8_t u8CommandByte;	// Last command byte received
} SFlashState;

static SFlashState sg_sFlashState;
...
static uint8_t sg_u8Buffer[100];
static uint8_t sg_u8BufferPos;
		...
		else
		{
			...
			else
			{
...

				{
					...
					sg_u8Buffer[sg_u8BufferPos++] = u8Data;
					if (sg_u8BufferPos >= sizeof(sg_u8Buffer))
					{
						psFile = fopen("u:/serial.txt", "a+");
						fwrite(sg_u8Buffer, 1, sizeof(sg_u8Buffer), psFile);
						fclose(psFile);
						sg_u8BufferPos = 0;
					}
				}
...
static EStatus EmulatorReload(void)
{
	...
	eStatus = LoadROM("r:/Nexus/Platform/Roscoe/RoscoeFlash.bin", 0, sg_u8FlashLoader, 512*1024);
...
//		*((uint64_t *) &sg_u8DRAM[u32Loop]) = SharedRandomNumber();
	...
	memset((void *) &sg_sFlashState, 0, sizeof(sg_sFlashState));
	sg_sFlashState.eState = EFLASH_NORMAL;
}
...
unsigned int  m68k_read_memory_8(unsigned int address)
{
	if ((address >= BASE_FLASH_LOADER) && ((address) < BASE_FLASH_BIOS))
	{
		...
		if (EFLASH_CMD == sg_sFlashState.eState)
		{
			// We're in flash command state
			if (0x90 == sg_sFlashState.u8CommandByte)
			{
				// Device ID and such
				if (0 == address)
				{
			...
			// This is what it returns when we don't know what it is
			return(0x55);
		}
...
void m68k_write_memory_8(unsigned int address, unsigned int value)
{
	...
	if ((address >= BASE_FLASH_LOADER) && ((address) < BASE_FLASH_BIOS))
	{
		DebugOut("Flash write=0x%.6x - %.2x\n", address, value);
		switch (sg_sFlashState.eState)
		{
			case EFLASH_NORMAL:
			{
				if ((0x5555 == address) && (0xaa == value))
				{
					sg_sFlashState.eState = EFLASH_BYTE1;
				}
				else
				{
					sg_sFlashState.eState = EFLASH_NORMAL;
			...
			case EFLASH_BYTE1:
			{
				if ((0x2aaa == address) && (0x55 == value))
				{
					sg_sFlashState.eState = EFLASH_BYTE2;
				}
				else
				{
					sg_sFlashState.eState = EFLASH_NORMAL;
			...
			case EFLASH_BYTE2:
			{
				if (0x5555 == address)
				{
					sg_sFlashState.eState = EFLASH_CMD;
					sg_sFlashState.u8CommandByte = value;
				}
				else
				{
					sg_sFlashState.eState = EFLASH_NORMAL;
			...
			case EFLASH_CMD:
			{
				if ((0x5555 == address) && (0xaa == value))
				{
					sg_sFlashState.eState = EFLASH_BYTE3;
				}
			...
			case EFLASH_BYTE3:
			{
				if ((0x2aaa == address) && (0x55 == value))
				{
					sg_sFlashState.eState = EFLASH_BYTE4;
				}
				else
				{
					sg_sFlashState.eState = EFLASH_CMD;
			...
			case EFLASH_BYTE4:
			{
				if ((0x5555 == address) && (0xf0 == value))
				{
					sg_sFlashState.eState = EFLASH_NORMAL;
					sg_sFlashState.u8CommandByte = 0;
				}
				else
				{
					sg_sFlashState.eState = EFLASH_CMD;
	...
	if ((address >= BASE_8BIT_DEVICES) && (address < BASE_16BIT_DEVICES))
	{
		...
		if (2 == (address >> 8))
		{
			...
			DebugOut("Intmask 0x%.2x write = 0x%.2x\n", address & 0xf, value);
			if (value)
			{
				value = value;
			}
...
static const uint8_t sg_u8Data[] = {'h', 'e', 'l', 'p', 0x0d, 0x00};
static uint8_t sg_u8DataOut = 0;
static uint32_t sg_u32LoopCount = 0;
...
static void EmulatorEntry(void)
{
	...
	while (1)
	{
		...
		if (sg_u32LoopCount > 300)
		{
			if (sg_u8DataOut >= sizeof(sg_u8Data))
			{
			}
			else
			{
				UARTRXData(&sg_sUARTA,
						   sg_u8Data[sg_u8DataOut]);
				sg_u8DataOut++;
			}
		}
		else
