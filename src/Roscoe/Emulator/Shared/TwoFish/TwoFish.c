...
#include	"Shared/TwoFish/Table.h"
...
if (useAsm & 4)
	...
	switch (keyLen)	/* case out key length for speed in generating S-boxes */
		...
		case 128:
			...
			#define	sb128(N) {						\
				Xor256((void *) L0,(void *) p8(N##2),b##N(sKey[1]));	\
		...
		case 192:
			...
			#define	sb192(N) {						\
				Xor256((void *) L0,(void *) p8(N##3),b##N(sKey[2]));	\
				{ register DWORD k0=b##N(sKey[0]);	\
		...
		case 256:
			...
			#define	sb256(N) {										\
				Xor256((void *) L1,(void *) p8(N##4),b##N(sKey[3]));					\
				for (i=0;i<256;i+=2) {L0[i  ]=p8(N##3)[L1[i]];		\
									  L0[i+1]=p8(N##3)[L1[i+1]]; }	\
				Xor256((void *) L0,(void *) L0,b##N(sKey[2]));						\
				{ register DWORD k0=b##N(sKey[0]);					\
				  register DWORD k1=b##N(sKey[1]);					\
