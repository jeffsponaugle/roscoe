...
static void SoundSDLCallback(void *pvUserData,
							 ...
							 int len)
{
	SoundRender((uint16_t *) pu8Stream,
				true);
}

// Initializes SDL's sound engine
EStatus SoundSDLInit(uint32_t u32TargetSampleRate,
					 uint32_t u32RenderBufferSize,
					 uint32_t u32NumberOfChannels)
{
	...
	Syslog("SDL Audio Back-ends compiled in to SDL\n");

	for (s32Loop = 0; s32Loop < SDL_GetNumAudioDrivers(); s32Loop++)
	{
		Syslog("  %s\n", SDL_GetAudioDriver(s32Loop));
	}

	Syslog("Audio driver: %s (null means default)\n", SDL_GetCurrentAudioDriver());
	...
	{
		u32Index = (uint32_t) atol(CmdLineOptionValue("-soundindex"));
