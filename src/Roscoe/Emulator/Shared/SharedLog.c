...
static SLog* LoggerAllocate( char* pu8Basename )
{
	...
	u32BasenameLength = (uint32_t)strlen(pu8Basename);
...
static void LoggerFlush( SLog* psLog )
{
	...
	else
	{
		...
		if( eStatus != ESTATUS_OK )
		{
			...
			// If tail doesn't exist, save the tail of the list being restored
			if( NULL == psLog->psItemTail )
			{
				psLog->psItemTail = psItemTail;
			}
					 ...
					 ... )
{
	...
	*pu8Buffer = 0;
	...
	if( bAddTimestamp )
	{
		...
		u32Remaining -= (uint32_t)strlen(&u8Time[0]);
	...
	if( pu8Function )
	{
		...
		u32Remaining -= (uint32_t)strlen(pu8Function);
