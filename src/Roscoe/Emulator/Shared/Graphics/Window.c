...
static uint32_t sg_u32FrameBurst;
...
typedef struct SWindowMsg
{
	...
	uint64_t u64Data1;				// Generic data variable
	uint64_t u64Data2;				// Second data variable
...
volatile static uint8_t sg_u8FrameLockCount = 0;
...
// not ESTATUS_OK.
#define	WindowListSetLock(bLock, eStatusIncoming)	WindowListSetLockInternal(__FUNCTION__, (uint32_t) __LINE__, bLock, eStatusIncoming)
...
void WindowUpdatedInternal(char *peFilename,
						   uint32_t u32LineNumber)
{
//	DebugOutFunc("File='%s', line=%u\n", peFilename, u32LineNumber);
...
void WindowGetVirtualSize(uint32_t *pu32XSize,
						  uint32_t *pu32YSize)
{
...
{
	EStatus eStatus;
	float fMsecPerCallback = (float) ((uint32_t) pvCallbackValue);
	SWindowMsg sMsg;
...
static void WindowFrame(void)
{
	...
	// Let's see if there's anything to draw. If not, don't.
	...
	// psWindow is non-NULL if there is at least one visible window
	if (psWindow)
	{
		// We're starting a frame and we do have things to draw
		GraphicsStartFrame();
		bFrameStarted = true;
//		DebugOut("**** GRAPHICS FRAME ****\n");
...
static void MouseCoordinatesPhysicalToLogical(Sint32 s32SDLX,
											  ...
											  uint32_t *pu32YPos)
{
	uint32_t u32XDisplaySize;
	uint32_t u32YDisplaySize;
	uint32_t u32XVirtualSize;
	uint32_t u32YVirtualSize;
	float fXScale;
	float fYScale;
	...
	if (pu32XPos)
	{
		*pu32XPos = (uint32_t) (s32SDLX * fXScale);
	}

	if (pu32YPos)
	{
		*pu32YPos = (uint32_t) (s32SDLY * fYScale);
	}
}
...
static void WindowSetFocusMessage(SWindowMsg *psMsg)
{
	...
	DebugOutFunc("Window %.8x - %s focus\n", (uint32_t) psMsg->eHandle1, peDir);
}

// Figures out which window is present at the incoming coordinates
static EWindowHandle WindowGetHandleByCoordinates(int32_t s32XPos,
												  int32_t s32YPos)
{
	SWindow *psWindow;
	EWindowHandle eWindowHandle;
	...
	while (psWindow)
	{
		...
		if ((s32XPos < psWindow->s32XPos) || (s32XPos >= (psWindow->s32XPos + (int32_t) psWindow->u32XSize)))
		{
			// Outside horizontally
		}
		else
		if ((s32YPos < psWindow->s32YPos) || (s32YPos >= (psWindow->s32YPos + (int32_t) psWindow->u32YSize)))
		{
			// Outside vertically
		}
		else
...
static uint8_t sg_u8MouseButtonState;
			...
			switch (sEvent.type)
			{
				...
				case SDL_MOUSEMOTION:
				{
					...
					if ((sg_bMouseInWindow) && (sg_bKeyboardFocus))
					{
						...
						CLEAR_MSG_STRUCT(sMsg);
						sMsg.u64Data1 = ((uint64_t) (u32MouseY) << 32) |
										((uint32_t) (u32MouseX));
				...
				case SDL_MOUSEBUTTONUP:
				{
					...
					if ((sg_bMouseInWindow) && (sg_bKeyboardFocus))
					{
						...
						if (sMsg.u64Data1)
						{
							...
							sMsg.u64Data1 |= (((uint64_t) u32MouseX) << 8);
							sMsg.u64Data1 |= (((uint64_t) u32MouseY) << 40);
				...
				case SDL_MOUSEWHEEL:
				{
					...
					if ((sg_bMouseInWindow) && (sg_bKeyboardFocus))
					{
						...
						sMsg.u64Data1 = ((int32_t) sEvent.wheel.y);
				...
				case SDL_KEYUP:
				{
					if ((sg_bMouseInWindow) && (sg_bKeyboardFocus))
					{
						...
						sMsg.u64Data1 = (uint64_t) sEvent.key.keysym.scancode;
										...
										int32_t s32MouseY)
{
	...
	SWindowMsg sMsg;
	int32_t s32MouseXOriginal;
	int32_t s32MouseYOriginal;
	...
	sMsg.u64Data1 |= (((uint64_t) s32MouseX) << 8);
	sMsg.u64Data2 = (((uint64_t) s32MouseY));
		...
		switch (sMsg.eMsg)
		{
			...
			case EWMSG_WINDOW_CREATE_GRAPHIC:
			{
				...
				eStatus = GraphicsCreateImage(sMsg.ppsGraphicsImage,
											  true,
											  (uint32_t) (sMsg.u64Data1),				// X Size
											  (uint32_t) (sMsg.u64Data1 >> 32));		// Y Size
			...
			case EWMSG_MOUSE_MOVEMENT:
			{
				...
				u32MouseX = (uint32_t) sMsg.u64Data1;
				u32MouseY = (uint32_t) (sMsg.u64Data1 >> 32);
				...
				if (ESTATUS_OK == eStatus)
				{
					...
					(void) ControlProcessDragMouseover(psWindow->eControlHandleFocus,
													   ...
													   ((int32_t) u32MouseX) - psWindow->s32XPos,
													   ((int32_t) u32MouseY) - psWindow->s32YPos);
				}
			...
			case EWMSG_MOUSE_BUTTON:
			{
				...
				s32MouseX = (int32_t) (sMsg.u64Data1 >> 8);
				s32MouseY = (int32_t) (sMsg.u64Data1 >> 40);
				...
				{
					// Pressed
					sg_u8MouseButtonState = (sg_u8MouseButtonState | ((uint8_t) sMsg.u64Data1));
				}
				else
				{
					// Released
					...
					if (sMsg.bGenericBoolean)
					{
						...
						if (eWindowHandle != ((EHandleGeneric) eInvalidHandle))
						{
							WindowButtonPressProcess(eWindowHandle,				// Which window got this press?
														(uint8_t) sMsg.u64Data1,	// Mouse button mask
...
//				DebugOut("Mouse button: Button=%u, State=%u, Total=%.2x, X=%u, Y=%u\n", (uint8_t) sMsg.u64Data1, sMsg.bGenericBoolean, sg_u8MouseButtonState, s32MouseX, s32MouseY);
			...
			case EWMSG_MOUSE_WHEEL:
			{
				...
				s32Wheel = ((int32_t) sMsg.u64Data1);
			...
			case EWMSG_CONTROL_BUTTON:
			{
				eStatus = ControlButtonProcess((EControlHandle) sMsg.eHandle1,
											   (uint8_t) sMsg.u64Data1,
											   (int32_t) (sMsg.u64Data1 >> 8),
											   (int32_t) (((int64_t) sMsg.u64Data2)),
											   sMsg.bGenericBoolean);
				break;
			}
			...
			case EWMSG_CONTROL_FOCUS:
			{
				...
				DebugOut("Control focus changed - %.8x (%s) %.8x (%s)\n", (uint32_t) sMsg.eHandle1, eSrcName, (uint32_t) sMsg.eHandle2, eDestName);
...
EStatus WindowRenderStretch(uint32_t *pu32RGBACornerImage,
							...
							uint32_t **ppu32RGBAWindowStretchImage)
{
	EStatus eStatus = ESTATUS_OK;
	uint32_t u32Loop;
	uint32_t *pu32RGBADest;
	uint32_t *pu32RGBADest2;
	uint32_t *pu32RGBASrc;
