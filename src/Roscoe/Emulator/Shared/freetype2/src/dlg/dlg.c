...
void dlg_escape_sequence(struct dlg_style style, char buf[12]) {
	...
	switch(count) {
		case 1: _snprintf(buf, 12, "\033[%dm", nums[0]); break;
		case 2: _snprintf(buf, 12, "\033[%d;%dm", nums[0], nums[1]); break;
		case 3: _snprintf(buf, 12, "\033[%d;%d;%dm", nums[0], nums[1], nums[2]); break;
...
int dlg_fprintf(FILE* stream, const char* format, ...) {
	...
	ret = dlg_vfprintf(stream, format, args);
	va_end(args);
	return ret;
}
...
void dlg_generic_outputf(dlg_generic_output_handler output, void* data,
	...
	for(it = format_string; *it; it++) {
		if(*it != '%') {
			output(data, "%c", *it);
			continue;
		}

		next = *(it + 1); // must be valid since *it is not '\0'
		if(next == 'h') {
			time_t t = time(NULL);
			struct tm tm_info;
			...
			bool first_tag = true;
			const char** tags;
			for(tags = origin->tags; *tags; ++tags) {
				if(!first_tag) {
					output(data, ", ");
				}
...
static void print_buf(void* dbuf, const char* format, ...) {
	...
	printed = vsnprintf(buf->buf, *buf->size, format, args);
	va_end(args);
...
bool dlg_remove_tag(const char* tag, const char* func) {
	struct dlg_data* data = dlg_data();
	unsigned int i;
	for(i = 0; i < vec_size(data->pairs); ++i) {
...
const char* dlg__printf_format(const char* str, ...) {
	...
	needed = vsnprintf(NULL, 0, str, vlist);
	if(needed < 0) {
	...
	buf = dlg_thread_buffer(&buf_size);
