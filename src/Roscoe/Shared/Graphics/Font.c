...
typedef struct SFontFile
{
	...
	UINT8 *pu8FontFile;				// Font file contents itself
	UINT64 u64FontFileSize;			// How big is the font file?
...
{
	UINT16 u16FontSize;				// Size of this font	
							...
							SFontChar **ppsFontChar)
{
	...
	(*ppsFontChar)->u32AdvanceX = (UINT32) (psFontFile->sTypeface->glyph->metrics.horiAdvance);
	(*ppsFontChar)->u32AdvanceY = (UINT32) (psFontFile->sTypeface->glyph->metrics.vertAdvance);
...
static void FontGeneratePixelTable(UINT32 u32RGBAColor,
								   UINT32 *pu32PixelTranslate,
								   UINT32 u32TranslateCount)
{
	UINT32 u32Loop;
	...
	for (u32Loop = 0; u32Loop < u32TranslateCount; u32Loop++)
	{
		pu32PixelTranslate[u32Loop] = (((u32RGBAColor & 0xff) * u32Loop) >> 8) |
									  (((UINT8) ((((u32RGBAColor >> 8) & 0xff)  * u32Loop) >> 8)) << 8) |
									  (((UINT8) ((((u32RGBAColor >> 16) & 0xff) * u32Loop) >> 8)) << 16) |
									  (((UINT8) (u32Loop)) << 24);
	}
}
...
static void FontRenderCharClipInternal(UINT32 *pu32RGBABufferBase,
									   ...
									   UINT32 *pu32PixelTranslate)
{
	INT32 s32XPosAdjusted = s32XPosBuffer + (INT32) (psFontChar->eHorizontalBearingX >> FT_FP_SHIFT);
	INT32 s32YPosAdjusted = s32YPosBuffer + (INT32) (psFontChar->eHorizontalBearingY >> FT_FP_SHIFT);
	INT32 s32XSizeAdjusted = (INT32) psFontChar->u32PixelXSize;
	INT32 s32YSizeAdjusted = (INT32) psFontChar->u32PixelYSize;
	...
	if ((s32XPosAdjusted + s32XSizeAdjusted) > (INT32) u32XSizeBuffer)
	{
		s32XSizeAdjusted = u32XSizeBuffer - s32XPosAdjusted;
	}

	// And bottom
	if ((s32YPosAdjusted + s32YSizeAdjusted) > (INT32) u32YSizeBuffer)
	{
	...
	if ((s32XSizeAdjusted > 0) &&
		(s32YSizeAdjusted > 0))
	{
		UINT8 *pu8SrcData = psFontChar->pu8FontPixelData;
		UINT32 *pu32RGBAPtr = pu32RGBABufferBase + s32XPosAdjusted + (s32YPosAdjusted * u32RGBABufferPitch);
		...
		if (pu8SrcData)
		{
			while (s32YSizeAdjusted)
			{
				...
				u32XSize = (UINT32) s32XSizeAdjusted;
				while (u32XSize)
				{
					UINT8 u8AlphaDest;

					u8AlphaDest = (UINT8) ((pu32PixelTranslate[*pu8SrcData] & 0xFF000000) >> 24);
					...
					else
					{
						...
						u8AlphaSrc = (UINT8) ((*pu32RGBAPtr & 0xFF000000) >> 24);
						 ...
						 UINT32 u32RGBAColor)
{
	...
	EUnicode eUnicodeChar;
	UINT32 u32MaxXTotal = 0;
	UINT32 u32MaxYTotal = 0;
	SFont *psFont = NULL;
	BOOL bLocked = FALSE;
	UINT32 u32PixelTranslate[0x100];
	...
	while (*peString)
	{
		...
		{
			UINT32 u32YSize = psFontChar->u32PixelYSize;
			UINT8 *pu8SrcData = psFontChar->pu8FontPixelData;
			UINT32 *pu32RGBAPtr = pu32RGBA;
...
{
	EStatus eStatus;
	UINT8 *pu8FontFile = NULL;
	UINT64 u64FontFileSize;
	SFontFile *psFontFile = NULL;
